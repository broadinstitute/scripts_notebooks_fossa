...
def get_example_to_name_metadata_cols(samples):
    ...
    print(f"\n Column's names: {metadata_cols} \n")

    return metadata_cols

def generate_csv(file_name, metadata_cols, sublist_add_platemap):

    output_folder = eg.diropenbox('Choose your output metadata folder', 'Choose folder')
    directory = "platemap"
    ...
    with open(rf"{path}\{file_name}.csv",'w',newline = '') as csvfile: #create a metadata.csv file 
    ...
    with open(rf"{path}\{file_name}.csv", 'r') as f_in, open(f"{path}\{file_name}.txt", 'w') as f_out:
